...
export default function Cart() {
  ...
  // ── Price calculations ───────────────────────────────────────────────────
  const deliverySettings = (store?.deliverySettings && typeof store.deliverySettings === 'object')
    ? store.deliverySettings as Record<string, unknown>
    : {};
  const freeDeliveryThreshold = Number(deliverySettings.freeAbove ?? 2000);
  const baseDeliveryCharge = Number(deliverySettings.standard ?? STORE_CONFIG.deliveryCharge);
  const freeDelivery = cartTotal >= freeDeliveryThreshold;
  const baseDelivery = freeDelivery ? 0 : baseDeliveryCharge;
  ...
  const goToCheckout = () => {
    try {
      if (appliedCoupon) {
        sessionStorage.setItem(CHECKOUT_COUPON_KEY, JSON.stringify(appliedCoupon));
      } else {
  ...
  const validateCouponFromApi = async (code: string, subtotal: number, deliveryCharge: number): Promise<AppliedCoupon> => {
    const url = new URL(`${API_URL}/coupons/validate`);
    url.searchParams.set('code', code);
    url.searchParams.set('store_slug', storeSlug);
    url.searchParams.set('total', String(subtotal));
    const resp = await fetch(url.toString());
    const data = await resp.json().catch(() => ({}));
    if (!resp.ok || !data?.valid) {
      throw new Error(data?.error ?? 'Invalid coupon');
    }
    const couponTypeRaw = String(data?.coupon?.type ?? 'fixed');
    const couponType = couponTypeRaw === 'percent' ? 'percentage' : couponTypeRaw;
    const discount = Number(data?.discount ?? 0);
    const finalDelivery = couponType === 'free_delivery' ? 0 : deliveryCharge;
    return {
      code,
      coupon: {
        code,
        discount_type: couponType as 'percentage' | 'fixed' | 'free_delivery',
        discount_value: Number(data?.coupon?.value ?? discount),
        min_order_amount: Number(data?.coupon?.minOrder ?? 0),
        max_uses: 0,
        expiry_date: null,
        active: true,
        description: 'API coupon',
      },
      discount,
      finalDelivery,
    };
        ...
        <div className="w-full md:w-80 lg:w-96 flex-shrink-0">
          <div className="bg-muted/30 border border-border rounded-xl p-6 sticky top-20">
            ...
            <CouponInput
              ...
              validateWithApi={validateCouponFromApi}
            />
