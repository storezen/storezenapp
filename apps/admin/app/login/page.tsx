...
export default function AdminLoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!resp.ok) {
        setError(String(data?.error ?? "Admin login failed"));
        return;
      }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader><CardTitle>Super Admin Login</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={onSubmit} className="space-y-3">
            <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ADMIN_EMAIL" required />
